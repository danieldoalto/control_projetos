...
def run_scan(config: AppConfig, db: Database) -> ScanResult:
    ...
    with db.get_connection() as conn:
        ...
        for root_path in roots:
            ...
            # 1. Descoberta de entidades na raiz
            ...
            # 2. Processamento de cada entidade descoberta
            for entity in discovered_entities:
                entity_path_str = str(entity.path)
                discovered_paths_in_root.add(entity_path_str)
                ...
                if macro_status == MacroStatus.NEW:
                    ...
                    if saved_entity and saved_entity.id:
                        # Atualiza apenas a data do último scan
                        entity_repo.update_fingerprint(
                            entity_id=saved_entity.id,
                            fingerprint=current_fp,
                            status=saved_entity.status,  # Preserva status atual (ex: analyzed ou unchanged)
                            last_scanned_at=now_ts,
                        )
            ...
            # 3. Detecção de entidades MISSING para esta raiz
